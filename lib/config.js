...
module.exports = {
  ...
  /* daily Ollama-release check — OFF by default (opt-in), so nothing leaves the
   * machine on a fresh install. MONKII_UPDATE_CHECK=on enables it. */
  UPDATE_CHECK: (process.env.MONKII_UPDATE_CHECK || 'off').toLowerCase() === 'on',
  ...
  OPENROUTER_KEY: (process.env.MONKII_OPENROUTER_KEY || '').trim(),
  ...
  OR_DATA_COLLECTION: (process.env.MONKII_OR_DATA_COLLECTION || 'deny').toLowerCase() === 'allow' ? 'allow' : 'deny',
  ...
  // the global JSON body parser (server.js) caps requests at 4mb; stay well
  // under that so an oversized write 413s cleanly instead of failing to parse
  WRITE_MAX_BYTES: 3 * 1024 * 1024,
  ...
  CHAT_MODEL_DEFAULT: process.env.MONKII_CHAT_MODEL || 'llama3.2', // small default chat model for first-run pull
  CHAT_MODEL_SIZE: '~2 GB', // approximate download size of the recommended chat model
