 ...
 * booting when CodeMonkii starts), with concurrent calls deduplicated.
 */
...
async function listModels() {
  ...
  return (data.models || []).map(m => ({ name: m.name, size: m.size, family: m.details && m.details.family }));
}
...
function streamChat({ model, messages, options, signal }) {
...
module.exports = { getVersion, listModels, streamChat, checkOllamaUpdate };
