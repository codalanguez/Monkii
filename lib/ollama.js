...
/* Embed one or more texts on-device via /api/embed. `input` may be a string
 * or an array of strings; returns an array of vectors (number[][]). Generous
 * timeout — a cold embed model loads into memory on the first call. */
async function embed(model, input, { signal, timeoutMs = 120000 } = {}) {
  ...
  if (!r.ok) {
    let detail = '';
    try { detail = (await r.json()).error || ''; } catch { /* raw */ }
    throw new Error(detail || `Ollama embed error ${r.status}`);
  }
  const data = await r.json();
...
module.exports = {
  getVersion, listModels, showModel, streamChat, chatOnce, embed,
