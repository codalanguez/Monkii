...
### Preview & save files
Click any file's name in the browser (📎, or Knowledge → Attach) to **preview** it before attaching — markdown renders like a chat reply, other text shows as-is, and binary files are refused rather than dumped as garbage. Right-click any message → **Save as file…** writes it to disk: pick a folder through the same browser, type a name (Monkii suggests one from the first line), and it's saved as real markdown — the original source, not a flattened copy, so headings, code fences, and links all survive. An existing file is never silently overwritten; you're asked first. Both are fenced by the same `MONKII_FS_ROOTS` allowlist as attachments — a save can't land, and a preview can't read, outside it.
